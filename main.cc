...
extern "C" void handler_get_username(GtkWidget *widget, GdkEvent *event, gpointer user_data){
  ...
  if (strlen(var_user_buffer_insert) == 0) {
    ...
    add_user (nome_utente, master_password);
...
extern "C" void handler_get_entry (GtkWidget *widget, GdkEvent *event, gpointer user_data){
  ...
  switch (gtk_switch_get_active(GTK_SWITCH(switch_generate))) {
    case TRUE:
      if (gtk_entry_get_text_length (GTK_ENTRY(website_generated_password)) != 0) {
        ...
        if (!add_entry(nome_utente, var_buffer_insert_title, var_buffer_insert_username, var_buffer_insert_password,
